...
def socksend(msg,ip_addr,port):
    try:
        clientsocket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)#DGRAM vs STREAM
...
if real_or_fake == 1: #real
    ...
    angle = 5.40390060322
    msg = str(time.time())+','+str(angle)+','+str(node_number)+',school.testing.10.08.16,$GPGGA,205704.000,3849.3529,N,07701.5355,W,1,08,1.2,10.0,M,-33.5,M,,0000*54'
    socksend(msg,ip_addr,port)

shot_bin = read_float_binary("luger_9mm");
shot = shot_bin(1:6000);
shot_lin = transpose(shot);
shot_match_filt = fliplr(shot_lin);
sens_input = read_float_binary("luger_9mm_indoors_crowd_shot");
sens_out = conv(sens_input,shot_match_filt);
csvwrite("/home/ben/Desktop/senior_design/shot_taps/luger_9mm_indoors_taps.txt",shot_match_filt)
(fliplr(shot_lin))(1:6)
shot_match_filt(1:6)
size(shot_bin)
size(shot)
...
plot(sens_out)
title("sensor output")
r = dlmread("luger_9mm_indoors_taps.txt", ",");
size(r)
r(1:6)
figure(6)
plot(r)